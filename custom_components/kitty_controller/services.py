...
from .const import (
    ...
    UPDATE_RULE_PROVIDER_SERVICE_NAME,
    HEALTHCHECK_PROVIDER_SERVICE_NAME,
    UPDATE_PROXY_PROVIDER_SERVICE_NAME,
)
...
async def async_setup_services(
    ...
    for coord in coordinators or []:
        proxy_providers.update(
            (coord.data.provider_proxies.get("providers") or {}).keys()
        )
        rule_providers.update(
            (coord.data.provider_rules.get("providers") or {}).keys()
        )
    proxy_options = sorted(proxy_providers)
    rule_options = sorted(rule_providers)
    # 选项为空（数据未就绪）时退回自由文本输入，避免 vol.In([]) 拒绝所有取值
    provider_selector = vol.In(proxy_options) if proxy_options else cv.string
    rule_selector = vol.In(rule_options) if rule_options else cv.string
    # 前端 UI 的字段选择器描述（与 voluptuous schema 分离，见 async_set_service_schema）
    provider_selector_desc = (
        {"select": {"options": proxy_options}} if proxy_options else {"text": {}}
    )
    rule_selector_desc = (
        {"select": {"options": rule_options}} if rule_options else {"text": {}}
    )

    update_rule_provider_schema = vol.Schema({
        vol.Required(CONF_DEVICE_ID): cv.string,
        vol.Required(ATTR_PROVIDER_NAME): rule_selector,
    })
    update_proxy_provider_schema = vol.Schema({
        vol.Required(CONF_DEVICE_ID): cv.string,
        vol.Required(ATTR_PROVIDER_NAME): provider_selector,
    })
    healthcheck_provider_schema = vol.Schema({
        vol.Required(CONF_DEVICE_ID): cv.string,
        vol.Required(ATTR_PROVIDER_NAME): provider_selector,
    })
    ...
    async def handle_reboot_core(call: ServiceCall) -> None:
        ...
        await coordinator.api.async_request("POST", "restart", json_data=RESTART_BODY)
    ...
    async def handle_get_rule(call: ServiceCall) -> ServiceResponse:
        coordinator = _get_coordinator(call.data[CONF_DEVICE_ID])
        response = await coordinator.api.async_request("GET", "rules")
    ...
    async def handle_update_rule_provider(call: ServiceCall) -> ServiceResponse:
        """触发指定规则集合重新拉取订阅内容（PUT providers/rules/{name}）。"""
        coordinator = _get_coordinator(call.data[CONF_DEVICE_ID])
        name = call.data[ATTR_PROVIDER_NAME].strip()
        if not name:
            raise HomeAssistantError("provider_name 不能为空")
        await coordinator.api.async_request("PUT", f"providers/rules/{quote(name, safe='')}")
        return {"provider": name}

    async def handle_update_proxy_provider(call: ServiceCall) -> ServiceResponse:
        """触发指定代理集合重新拉取订阅内容（PUT providers/proxies/{name}）。"""
        coordinator = _get_coordinator(call.data[CONF_DEVICE_ID])
        name = call.data[ATTR_PROVIDER_NAME].strip()
        if not name:
            raise HomeAssistantError("provider_name 不能为空")
        await coordinator.api.async_request("PUT", f"providers/proxies/{quote(name, safe='')}")
        return {"provider": name}
    ...
    async def handle_healthcheck_provider(call: ServiceCall) -> ServiceResponse:
        ...
        # 触发集合级测速（无参数，使用集合自身配置的 healthCheck URL 与超时）
        await coordinator.api.async_request("GET", f"providers/proxies/{encoded}/healthcheck")
        # 回读集合详情，提取各节点存活状态与最近一次延迟
        data = await coordinator.api.async_request("GET", f"providers/proxies/{encoded}")
        raw_proxies = (data or {}).get("proxies") or []
        if isinstance(raw_proxies, dict):
            proxy_items = list(raw_proxies.values())
        else:
            proxy_items = list(raw_proxies)

        nodes: dict[str, Any] = {}
        alive_count = 0
        for node in proxy_items:
            ...
            if alive:
                alive_count += 1
            nodes[str(node_name)] = int(delay) if isinstance(delay, (int, float)) else -1
    ...
    hass.services.async_register(
        DOMAIN, HEALTHCHECK_PROVIDER_SERVICE_NAME, handle_healthcheck_provider,
        schema=healthcheck_provider_schema, supports_response=SupportsResponse.ONLY
    )
    ...
    rule_fields = {
        "device_id": {"selector": {"device": {"integration": DOMAIN}}},
        "provider_name": {"selector": rule_selector_desc},
    }
    async_set_service_schema(
        hass, DOMAIN, UPDATE_RULE_PROVIDER_SERVICE_NAME, {"fields": rule_fields}
    )
    async_set_service_schema(
        hass, DOMAIN, UPDATE_PROXY_PROVIDER_SERVICE_NAME, {"fields": provider_fields}
    )
    async_set_service_schema(
        hass, DOMAIN, HEALTHCHECK_PROVIDER_SERVICE_NAME, {"fields": provider_fields}
    )